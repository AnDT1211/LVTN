...
roadgbt_sug = RoadGBT(INIT_RoadGBT_SUG(init_params));
roadgbt = RoadGBT(INIT_RoadGBT(init_params));

%% Plot road Profile
Yirc = roadgbt_sug.dis_t(init_params.t);
...
plot(S, Ygbt ,'r')
hold off;
legend(roadgbt_sug.roadtype, roadgbt.roadtype);
xlim([0 roadgbt_sug.d1])
grid on
grid minor
...
saveas(figure(1), 'img/Compare/GBTvsGBT_SUG/r_road.png');
...
solveT_IRC = SolverT(init, roadgbt_sug, airspring);
...
leg1 = strcat('GB/T suggestion non-linear');
leg2 = strcat('GB/T suggestion linear');
...
xlabel('Time (s)');
ylabel('Displacement (m)');
saveas(figure(1), 'img/Compare/GBTvsGBT_SUG/a1_dis.png');
...
saveas(figure(2), 'img/Compare/GBTvsGBT_SUG/a2_revdis.png');
...
saveas(figure(3), 'img/Compare/GBTvsGBT_SUG/a3_acc.png');
...
hold off
legend({leg1, leg2, leg3, leg4}, 'Position',[0.72, 0.64, 0, 0]);
grid on
grid minor
...
ylabel('BVA (m/s^2)');
saveas(figure(1), 'img/Compare/GBTvsGBT_SUG/v1_BVA.png');
...
saveas(figure(2), 'img/Compare/GBTvsGBT_SUG/v2_RMSA.png');
...
xlabel('Car speed (km/h)');
ylabel('SDD (m)');
saveas(figure(3), 'img/Compare/GBTvsGBT_SUG/v3_SDD.png');
...
xlabel('Car speed (km/h)');
ylabel('TDL (N)');
saveas(figure(4), 'img/Compare/GBTvsGBT_SUG/v4_TDL.png');
