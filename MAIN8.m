...
road = RoadIRC(INIT_RoadIRC_SUG(init_params));
...
solveT.SolvingTimeResponse(VLin(1));


% [~, idxMaxA_non] = findpeaks(-1*solveT.a_non);
% [~, idxMaxA_lin] = findpeaks(-1*solveT.a_lin);
% figure(1)
% plot(solveT.T, solveT.a_non,'r')
% hold on
% plot(solveT.T(idxMaxA_non), solveT.a_non(idxMaxA_non),'ro')
% plot(solveT.T, solveT.a_lin,'b')
% plot(solveT.T(idxMaxA_lin), solveT.a_lin(idxMaxA_lin),'bo')
% plot(solveT.T, road.acc_t(solveT.T));
% plot(solveT.T, road.dis_t(solveT.T));
% 
...
SOLVE = SolverFnZt(VLin, INIT_FnZtIRC_SUG(), solveT);
